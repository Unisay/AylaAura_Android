...
public class NetThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            mdnsListener.failed(new PreconditionError("WiFi is not enabled."));
            return;
        }
        try {
            ...
            mdnsListener.failed(new AylaError(AylaError.ErrorType.Internal, "Cannot open socket " +
                    "on port "+MDNS_AYLA_PORT));
            return;
        }
        mdnsListener.ready();
        ...
        while (true) {
            ...
            try {
                ...
                if (cmd == null) {
                    ...
                    mdnsListener.failed(new AylaError(AylaError.ErrorType.AylaError,
                            "IOException in receive response ",e));
                    return;
                }
                ...
                try {
                    ...
                    mdnsListener.failed(new AylaError(AylaError.ErrorType.AylaError,
                            "IOException during socket reopen ",e1));
                    return;
                }
                ...
                if (cmd instanceof QueryCommand) {
                    try {
                        ...
                        mdnsListener.failed(new AylaError(AylaError.ErrorType.AylaError,
                                "IOException during query command ",e1));
                    }
