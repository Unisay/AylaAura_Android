...
public class AylaWeChatAuthProvider extends AylaOAuthProvider {
    ...
    public AylaWeChatAuthProvider(String wechatAppId) {
        if (__myInstance != null) {
            throw new RuntimeException("Still waiting for response...");
        }
        __myInstance = this;
    ...
    public static void activityCancelAuth() {
        if (__myInstance != null) {
            if(__myInstance._listener != null) {
                __myInstance._listener.didFailAuthentication(new AuthError("User canceled request", null));
            }
            __myInstance = null;
        }
    }
    ...
    public void authenticate(final AuthProviderListener listener) {
        ...
        IWXAPI api = WXAPIFactory.createWXAPI(context, _wechatAppId, false);
        api.sendReq(req);
    }
