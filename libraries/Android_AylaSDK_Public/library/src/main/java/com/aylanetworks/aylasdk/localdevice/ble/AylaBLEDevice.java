...
public class AylaBLEDevice extends AylaLocalDevice {
    ...
    protected final List<Response.Listener<AylaAPIRequest.EmptyResponse>>
            _pendingConnectListeners = new ArrayList<>();
    ...
    public AylaBLEDevice(BluetoothDevice discoveredDevice, int rssi, byte[] scanData) {
        super();
        AylaLog.d(LOG_TAG, "Constructor: discoveredDevice: " + discoveredDevice + " rssi:" + rssi +
    ...
    public boolean requiresLocalConfiguration() {
        ...
        // avialable) to the BD_ADDR that we need to connect to the device.
     ...
     * application the bluetooth mappins will be available without requiring a scan.
     *
    ...
    public String getBluetoothAddress() {
        ...
        SharedPreferences prefs = AylaNetworks.sharedInstance().getContext()
                .getSharedPreferences(PREFS_ADDRESS_MAPPING, Context.MODE_PRIVATE);
        return prefs.getString(PREFS_ADDR_PREFIX + unique_hardware_id, null);
    }
    ...
    protected void onCharacteristicsFetched() {
        // Let anybody waiting for connectLocal() that we're done connecting
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            @Override
            public void run() {
                for (Response.Listener<AylaAPIRequest.EmptyResponse> listener :
                        _pendingConnectListeners) {
        ...
        // Check the cloud service for any queued commands
        checkQueuedCommands();
    }
    ...
    public AylaAPIRequest connectLocal(final Response.Listener<AylaAPIRequest.EmptyResponse> successListener,
        ...
        if (_isConnected) {
            AylaLog.d(LOG_TAG, this.toString() + ": connectLocal: already connected");
        ...
        if (_bluetoothDevice != null) {
            ...
            _pendingConnectListeners.add(successListener);
            _bluetoothGatt = _bluetoothDevice.connectGatt(AylaNetworks.sharedInstance().getContext(),
                    true, new AylaGattCallback(this));

            // Return an AylaAPIRequest that the caller can use to cancel notification of connection
            return new AylaAPIRequest<AylaAPIRequest.EmptyResponse>(Request.Method.GET, null, null,
                    AylaAPIRequest.EmptyResponse.class, null, successListener, errorListener) {
                @Override
                public void cancel() {
                    _pendingConnectListeners.remove(successListener);
                }
            };
        }
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        AylaLog.d(LOG_TAG, "Connection state change [" + getFriendlyName() + "]: " + newState);
    ...
    public void onCharacteristicRead(BluetoothGatt gatt,
        ...
        if (status == BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION ||
            ...
            // We need to pair with this deviceAuraLocalDeviceManager
    ...
    @Override
    public AylaAPIRequest updateProductName(final String displayName, final Response.Listener<AylaAPIRequest.EmptyResponse> successListener, final ErrorListener errorListener) {
