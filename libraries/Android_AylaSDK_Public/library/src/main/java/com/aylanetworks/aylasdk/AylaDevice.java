...
public class AylaDevice {
    ...
    @Expose
    protected String deviceName;              // name of this device
    @Expose
    ...
    public interface DeviceChangeListener {
         ...
         */
        void deviceLanStateChanged(AylaDevice device, boolean lanModeEnabled);
    }
    ...
    /**
     * Returns a friendly name for this device. If the device name is set, it will be returned.
     * If not, and the product name is set, that will be returned. Otherwise the device's DSN will
     * be returned.
     *
     * @return a friendly name for this device
     */
    public String getFriendlyName() {
        String response = getDeviceName();
        ...
        if (lanEnabled && getSessionManager() != null && !getSessionManager().isCachedSession()) {
            ...
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                ...
                public void run() {
                    synchronized (_deviceChangeListeners) {
                        for (DeviceChangeListener listener : _deviceChangeListeners) {
                            listener.deviceLanStateChanged(AylaDevice.this, lanEnabled);
                        }
                    }
                }
            });
     ...
     */
    protected void mergeProperties(AylaProperty[] fetchedProperties) {
        List<Change> propertyChanges = new ArrayList<>();

        synchronized (_propertyMap) {
            // Add or update our properties with what was passed in
            for (AylaProperty fetchedProperty : fetchedProperties) {
    ...
    public Change updateFrom(AylaDevice other, DataSource source) {
        ...
        if (!ObjectUtils.equals(other.deviceName, deviceName)) {
