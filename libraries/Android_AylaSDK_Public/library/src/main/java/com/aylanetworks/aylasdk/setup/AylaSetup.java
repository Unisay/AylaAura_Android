...
public class AylaSetup {
     ...
     */
    private final static int REQ_TIMEOUT_POLL_WIFI_STATUS = 10000;
    ...
    final private Set<DeviceWifiStateChangeListener> _wifiStateChangeListeners;
    ...
    public AylaAPIRequest scanForAccessPoints(
            ...
            final Predicate<ScanResult> filter,
    ...
    public AylaAPIRequest scanAPsWithRegex(int timeoutInSeconds,
                                              ...
                                              final ErrorListener errorListener){
        return this.scanForAccessPoints(timeoutInSeconds, new Predicate<ScanResult>() {
            @Override
    ...
    public AylaAPIRequest connectToNewDevice(final String ssid,
        ...
        if (ObjectUtils.unquote(currentNetworkInfo.getSSID()).equals(ssid)) {
            // Already on the requested network. Just get the device details.
            return fetchDeviceDetail(null, deviceDetailsSuccessListener, deviceDetailsErrorListener);
        ...
        if (configs != null) {
            for (WifiConfiguration config : configs) {
                if (TextUtils.equals(config.SSID, conf.SSID)) {
                    netId = config.networkId;
                    AylaLog.d(LOG_TAG, "Found existing netId " + netId);
                    break;
                }
            }
        }
                ...
                new AylaConnectivity.AylaConnectivityListener() {
                    ...
                    public void connectivityChanged(boolean wifiEnabled, boolean cellularEnabled) {
                        ...
                        AylaLog.d(LOG_TAG, "Compare with: " + wifiEnabled + "info.ssid: " +
                                ObjectUtils.unquote(info.getSSID()) + " and ssid: "+ssid);
                        if (wifiEnabled && ObjectUtils.unquote(info.getSSID()).equals(ssid)) {
                            timeoutHandler.removeCallbacksAndMessages(null);
                            ...
                            AylaLog.d(LOG_TAG, "Connected to " + ObjectUtils.unquote(info.getSSID
                                    ()) + ", want" + " to " + "connect to " + ssid);
                        }
                    }
                };
        ...
        timeoutHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (info != null && ObjectUtils.unquote(info.getSSID()).equals(ssid)) {
                    timeoutHandler.removeCallbacksAndMessages(null);
    ...
    private AylaError bindToNetwork() {
        ...
        AylaLog.d(LOG_TAG, "Looking for DHCP address...");
        ...
        AylaLog.d(LOG_TAG, "DHCP info: " + Formatter.formatIpAddress(dhcpInfo.ipAddress));
        if (dhcpInfo.ipAddress == 0) {
            AylaLog.e(LOG_TAG, "Failed to get DHCP address");
            return new PreconditionError("Failed to get Ip address from " +
                    "device.") ;
        }
        AylaLog.d(LOG_TAG, " Mobile got IPAddress "+dhcpInfo.ipAddress);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (foundNetwork != null) {
                AylaLog.d(LOG_TAG, "foundNetwork : "+foundNetwork);
        ...
        AylaLog.d(LOG_TAG, "returning successListener of bindToNetwork() ");
        return null;
    }
     ...
     */
    public AylaAPIRequest fetchDeviceAccessPoints(final Predicate<AylaWifiScanResults.Result> filter,
    ...
    public AylaAPIRequest fetchDeviceAPsWithRegex(final String scanRegex,
        ...
        return this.fetchDeviceAccessPoints(new Predicate<AylaWifiScanResults.Result>() {
            @Override
                    ...
                    public void onResponse(AylaWifiStatus response) {
                       ...
                       updateAndNotifyState(response.getState());
                        if (TextUtils.equals(response.getConnectedSsid(), ssid)){
                            if( response.getConnectHistory()[0].error ==
                            ...
                            if ((history != null) && (history.length > 0))
                            {
                                ...
                                if(_lastWifiStatus != null) {
                                    ...
                                    if (lastFetchedHistory != null &&
                                        ...
                                        timeoutHandler.removeCallbacksAndMessages(null);
                                        connectivity.unregisterListener(connectivityListener);
                                        errorListener.onErrorResponse(new InternalError(errorMsg));
                                    }
                                }
                            }
    ...
    public AylaAPIRequest reconnectToOriginalNetwork(int timeoutInSeconds,
        ...
        if (_currentNetworkInfo == null) {
            errorListener.onErrorResponse(new PreconditionError("No original network information " +
                    "found. Did you change networks via AylaSetup?"));
            return null;
        }
        ...
        if (wifiManager.getConnectionInfo().getSSID().equals(_currentNetworkInfo.getSSID())) {
            successListener.onResponse(new EmptyResponse());
            return null;
        }
        ...
        if (wifiInfo != null) {
            AylaLog.d(LOG_TAG, "Forgetting module SSID " + wifiInfo.getSSID());
        ...
        if (configs != null) {
            for (WifiConfiguration config : configs) {
                if (ObjectUtils.unquote(config.SSID)
                        .equals(ObjectUtils.unquote(_currentNetworkInfo.getSSID()))) {
                    netId = config.networkId;
                    break;
                }
            }
        ...
        if (netId == NETID_UNKNOWN) {
            ...
            errorListener.onErrorResponse(new PreconditionError("Unable to find original network " +
                    "with SSID " + _currentNetworkInfo.getSSID()));
            return null;
        }
                    ...
                    public void connectivityChanged(boolean wifiEnabled, boolean cellularEnabled) {
                        if (wifiEnabled) {
                            ...
                            if (info != null && _currentNetworkInfo != null) {
                                if (ObjectUtils.unquote(info.getSSID())
                                      .equals(ObjectUtils.unquote(_currentNetworkInfo.getSSID()))) {
                                    // We're all set.
                                    timeoutHandler.removeCallbacksAndMessages(null);
                                    connectivity.unregisterListener(this);
        ...
        timeoutHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (_currentNetworkInfo != null) {
                    result = _currentNetworkInfo.getSSID();
                }
    ...
    private AylaAPIRequest setDeviceTime(Date deviceTime,
        ...
        try {
            time.put("time", deviceTime.getTime());
        ...
        AylaAPIRequest<AylaSetupDevice> request = new AylaAPIRequest<AylaSetupDevice>(
            ...
            parseNetworkResponse(NetworkResponse networkResponse) {
               ...
               // Todo: AylaLogService.setDsn(_setupDevice.getDsn());
                ...
                // TODO: Uncomment this and test it well. Too close to release to enable any
                // changes to WiFi setup.
//                EmptyListener<EmptyResponse> dontCare = new EmptyListener<>();
//                setDeviceTime(null, dontCare, dontCare);
