...
public class RobDeviceDatumTest {
    ...
    public void testFetchDatumWithKey(){
        ...
        try {
            fetchedDatum = futureFetchdatum.get(RobTestConstants.API_TIMEOUT_MS,
    ...
    public void testFetchDatumList(){
        ...
        try {
            datumList = futureFetch.get(RobTestConstants.API_TIMEOUT_MS, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            fail("Error in device datum creation "+e);
        } catch (ExecutionException e) {
            fail("Error in device datum creation " + e);
